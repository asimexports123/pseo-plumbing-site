...
async function testAllUrls() {
  ...
  for (const city of SEED_CITIES) {
    ...
    for (const service of SERVICES) {
      ...
      if (service.slug === 'water-softener-repair') {
        waterSoftenerUrls.push(url);
      }
  ...
  console.log(`\nWater Softener Repair URLs (${waterSoftenerUrls.length}):`);
  waterSoftenerUrls.forEach(url => console.log(`  - ${url}`));
}
