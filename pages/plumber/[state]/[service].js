...
export async function getStaticProps({ params }) {
  try {
    ...
    const lastReviewed = await getPageDate(`state-service:${stateObj.slug}:${serviceObj.slug}`);
          ...
          <div className="flex flex-col md:flex-row md:items-stretch">
            <div className="relative z-10 flex-1 flex items-center px-4 sm:px-6 lg:px-12 py-10 md:py-20 lg:py-14">
              <div className="w-full max-w-2xl mx-auto md:mx-0 text-center md:text-left">
                <div className="inline-block bg-red-600 text-sm font-bold px-3 py-1 rounded-full mb-3">⚡ Available 24/7 — All of {stateObj.name}</div>
                ...
                <a href="tel:1" className="inline-flex items-center gap-3 bg-red-600 hover:bg-red-500 text-white px-8 py-4 rounded-full text-xl font-extrabold shadow-xl transition-transform hover:scale-105 no-underline" aria-label="Call emergency dispatch">
                  📞 Get Help Now
                </a>
        ...
        <main className="max-w-4xl mx-auto w-full px-4 py-10">
          ...
          {cityCards.length === 0 && (
            <div className="mb-10 bg-blue-50 border border-blue-200 rounded-xl p-6 text-center">
              ...
              <a href="tel:1" className="inline-flex items-center gap-2 bg-red-600 text-white px-6 py-3 rounded-full font-bold">
                📞 Check Availability
              </a>
            </div>
          )}
