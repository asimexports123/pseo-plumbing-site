...
async function testAllUrls() {
  ...
  for (const city of SEED_CITIES) {
    ...
    for (const service of SERVICES) {
      ...
      if (service.slug === 'garbage-disposal-repair') {
        garbageDisposalUrls.push(url);
      }
  ...
  console.log(`\nGarbage Disposal Repair URLs (${garbageDisposalUrls.length}):`);
  garbageDisposalUrls.forEach(url => console.log(`  - ${url}`));
}
