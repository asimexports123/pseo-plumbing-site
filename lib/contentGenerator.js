import { getCityData } from './cities-server';
import { getServiceBlocks } from './serviceContent';
import {
  pick, pickNum,
...
const PRICING_GUIDANCE = {
  ...
  'whole-house-repiping': {
    heading: 'What Does Whole-House Repiping Cost?',
    disclaimer: 'These ranges are educational estimates for whole-house repiping. Your actual quote in {city} depends on home size, number of fixtures, pipe material, access, wall restoration, and permit requirements. You receive a written scope and upfront quote before work begins.',
    items: [
      { label: 'Repiping assessment and pressure testing', low: '$150', high: '$400', note: 'System evaluation, pipe-material assessment, and replacement plan' },
      { label: 'Targeted pipe reroute', low: '$1,500', high: '$4,000', note: 'New accessible run to replace a repeatedly failing branch' },
      { label: 'Two-bathroom home repipe', low: '$4,000', high: '$9,000', note: 'Typical PEX or copper replacement scope; access and restoration vary' },
      { label: 'Larger or multi-story home repipe', low: '$8,000', high: '$18,000', note: 'More fixtures, longer runs, and access complexity increase scope' },
      { label: 'Drywall access and restoration coordination', low: '$500', high: '$3,000', note: 'Opening and restoration needs vary by routing approach and finish materials' },
    ],
  },
  'main-water-shutoff-valve-repair': {
    heading: 'What Does Main Water Shutoff Valve Repair Cost?',
    disclaimer: 'These ranges are educational estimates for main water shutoff valve work. Your actual quote in {city} depends on valve location, pipe material, corrosion, access, utility coordination, and whether replacement is required. All pricing is confirmed in writing before work begins.',
    items: [
      { label: 'Main valve inspection and diagnosis', low: '$75', high: '$175', note: 'Tests valve operation, identifies leakage, and confirms isolation options' },
      { label: 'Accessible interior shutoff valve replacement', low: '$150', high: '$450', note: 'Standard ball-valve replacement on accessible residential piping' },
      { label: 'Corroded or seized valve replacement', low: '$300', high: '$900', note: 'Additional cutting, fittings, and pipe restoration may be needed' },
      { label: 'Meter-adjacent valve coordination and repair', low: '$350', high: '$1,200', note: 'Scope varies by ownership boundary, access, and utility requirements' },
      { label: 'Emergency water isolation and repair', low: '$200', high: '$650', note: 'Urgent shutoff and replacement work for an active leak or failed valve' },
    ],
  },
};
...
export function generatePageContent(cityName, stateCode, service) {
  const fn = {
    ...
    'whole-house-repiping': genWholeHouseRepiping,
    'main-water-shutoff-valve-repair': genMainWaterShutoffValveRepair,
...
function buildInfraSection(cityName, stateCode, service) {
  ...
  const zipText = d.zipCodes && d.zipCodes.length > 0
    ? `We serve all ${cityName} zip codes including ${d.zipCodes.slice(0, 4).join(', ')}${d.zipCodes.length > 4 ? ', and surrounding areas' : ''}.`
    : `We serve throughout the ${cityName} metro and surrounding communities.`;
...
function dominantFailureSentence(cityName, d, svc) {
  ...
  const climateDetail = d.winterRisk === 'high'
    ...
    : d.summerRiskNote;
  const base = `In ${cityName}, the combination of ${tier} water at ${d.hardnessPpm} mg/L from the ${d.waterUtility}, ${d.pipeMaterial} in homes built ${d.pipeEra}, ${d.soilType}, and ${climateDetail} creates conditions where ${d.dominantFailure} is the most commonly diagnosed plumbing failure`;
  const tails = {
    'emergency': ` — a pattern that shapes how our emergency plumbers triage and prioritize every urgent call in ${cityName}.`,
    'water-heater-repair': ` — and that pattern directly influences the water heater failures our technicians diagnose most often in ${cityName}.`,
    'leak-repair': ` — and it is the same pattern our leak detection specialists look for first when tracing hidden moisture in ${cityName} homes.`,
    'drain-cleaning': ` — a pattern that informs which drain cleaning methods our technicians select for ${cityName} properties.`,
    'pipe-burst-repair': ` — and this failure pattern is exactly what our burst pipe repair team prepares for when responding to ${cityName} emergencies.`,
    'sewer-line-repair': ` — and it is the underlying pattern our sewer line specialists consider when diagnosing recurring backups in ${cityName}.`,
    'toilet-repair': ` — and this pattern helps our toilet repair technicians identify whether the issue is fixture-specific or systemic in ${cityName} homes.`,
    'slab-leak-repair': ` — and it is the pattern our slab leak detection team accounts for when evaluating foundation moisture in ${cityName}.`,
    'water-line-repair': ` — and this pattern guides our water line repair strategy when assessing main service line integrity in ${cityName}.`,
    'faucet-repair': ` — and it is the same mineral and corrosion pattern our faucet repair technicians find when disassembling fixtures in ${cityName} homes.`,
    'garbage-disposal-repair': ` — and this pattern affects how our garbage disposal repair team evaluates grinding and drainage performance in ${cityName}.`,
    'water-softener-repair': ` — and it is the water chemistry pattern our softener repair technicians reference when calibrating systems for ${cityName} homes.`,
    'whole-house-repiping': ` — and this pattern is often the deciding factor when our repiping specialists recommend full pipe replacement for ${cityName} homes.`,
    'main-water-shutoff-valve-repair': ` — and it is the pattern our valve repair technicians consider when evaluating shutoff reliability in ${cityName} homes.`,
    'sump-pump-repair': ` — and this pattern informs how our sump pump team assesses groundwater risk and pump sizing for ${cityName} basements.`,
  };
  const tail = tails[svc] || ` — a pattern that shapes how our technicians approach every service call in ${cityName}.`;
  return `${base}${tail}`;
}
...
const EMERGENCY_FAQ_POOL = [
  ...
  { q: (c,s) => `How fast can a licensed plumber reach my ${c} address?`,
    a: (c,s) => `Our target response time in ${c} is under 60 minutes for emergency calls. An operator confirms your address and sends the nearest available plumber immediately after your call.` },
  { q: (c,s) => `Are your plumbers licensed to work in ${s || 'my state'}?`,
    a: (c,s) => `Yes. Every plumber sent to ${c} holds an active ${s || 'state'} plumbing license and carries liability insurance. You can request to see credentials when they arrive.` },
  { q: (c,s) => `What should I do while waiting for the emergency plumber in ${c}?`,
    a: (c,s) => `If you have an active water leak, locate and shut your main water supply valve. For gas concerns, leave the building and call from outside. Our operator will walk you through any immediate safety steps.` },
  { q: (c,s) => `Can you handle sewer backups in ${c}, not just pipe failures?`,
    a: (c,s) => `Yes. Sewer backups are one of the most common emergency calls we receive in ${c}. We send plumbers equipped for sewer line clearing and camera inspection.` },
  { q: (c,s) => `Do you handle gas line emergencies in ${c}?`,
    a: (c,s) => `Yes. Licensed plumbers sent to ${c} are trained to respond to gas line concerns. If you smell gas, leave your home immediately and call from outside before contacting us.` },
  { q: (c,s) => `Is same-day emergency plumbing service available throughout ${c}?`,
    a: (c,s) => `Yes. We cover the full ${c} metro area with same-day emergency service, including weekends and holidays.` },
  ...
  { q: (c,s) => `Will a real plumber answer if I call at 3 AM from ${c}?`,
    a: (c,s) => `Yes. Our line is staffed 24 hours a day, 7 days a week. A live operator — not an answering machine — takes your call and sends a plumber immediately.` },
  { q: (c,s) => `How do I know the plumber sent to my ${c} home is trustworthy?`,
    a: (c,s) => `Every plumber we send to ${c} is licensed, insured, and background-checked. You're welcome to request license number verification when they arrive.` },
  ...
  { q: (c,s) => `Can I find an emergency plumber near me in ${c} at any time?`,
    a: (c,s) => `Yes. Our 24/7 service serves ${c} around the clock — nights, weekends, and holidays included. When you search for an emergency plumber near me, our local team is available to respond to your urgent plumbing needs with licensed plumbers familiar with ${c} neighborhoods.` },
  { q: (c,s) => `What makes you a 24 hour plumber in ${c}?`,
    a: (c,s) => `We maintain live operator and plumber availability 24 hours a day. Whether it's 3 AM on a Tuesday or midnight on a Sunday, our emergency plumber team in ${c} is ready to respond to your call with fully stocked service vehicles and licensed plumbers.` },
  { q: (c,s) => `Do you serve as a local emergency plumber throughout ${c}?`,
    a: (c,s) => `Yes. Our plumbers are local to ${c} and familiar with the area's plumbing infrastructure and common issues. We serve from within the region to provide fast response times for local emergency plumbing needs across all ${c} neighborhoods.` },
];
...
const HOW_STEPS = [
  ['Call our 24/7 emergency line', 'Operator confirms your location and issue', 'Nearest licensed plumber sent immediately', 'Plumber provides upfront quote on arrival', 'Repair completed — area left clean', 'Follow-up available if any concerns arise'],
  ['You call — a live person answers', 'We identify the problem and send a plumber within minutes', 'Plumber arrives with a stocked service vehicle', 'Diagnosis explained before any work begins', 'Repair carried out with your approval', 'Job documented for your records'],
  ['Live operator takes your call immediately', 'Plumber routed to your address in real time', 'On-site arrival — assessment begins', 'Written quote provided before any work starts', 'Repair performed and tested', 'You confirm satisfaction before plumber leaves'],
];
...
function genEmergency(cityName, stateCode, service) {
  ...
  return {
    ...
    sections: [
      {
        heading: h1,
        body: `Our ${cityName} emergency plumbing team is available around the clock to handle every crisis — from burst pipes and sewer backups to gas line concerns and failed water heaters. Plumbers arrive in fully stocked service vehicles carrying the parts needed for most common repairs on the first visit. ${trustPhrase}`,
        list: serviceList,
      },
      infraSections[0],
      infraSections[1],
      {
        ...
        list: [
          ...
          `Clear communication from call to completion`,
        ],
      },
      {
        heading: h3,
    ...
    blocks: getServiceBlocks('emergency', cityName, stateCode, d),
...
const LEAK_FAQ_POOL = [
  ...
  { q: (c,s) => `How quickly can you respond to a suspected leak in ${c}?`,
    a: (c,s) => `For active leaks causing water damage, we respond within the same response window as any emergency. For suspected hidden leaks without active flooding, we offer same-day and next-day assessment appointments throughout ${c}.` },
...
function genLeakRepair(cityName, stateCode, service) {
  ...
  return {
    ...
    blocks: getServiceBlocks('leak-repair', cityName, stateCode, d),
...
const DRAIN_FAQ_POOL = [
  { q: (c,s) => `Do you offer emergency drain service in ${c}?`,
    a: (c,s) => `Yes. Emergency drain service is available 24/7 in ${c}. If you have a severe clog, sewage backup, or multiple drains backing up simultaneously, we send a licensed plumber immediately — day or night. A live operator answers every call, and we aim to have a plumber on-site within 60 minutes.` },
  ...
  { q: (c,s) => `How fast can you respond for drain cleaning in ${c}?`,
    a: (c,s) => `For emergencies where water is actively backing up, we respond with the same urgency as any plumbing emergency. For routine drain cleaning appointments in ${c}, we offer same-day and next-day scheduling.` },
  ...
  return {
    ...
    sections: [
      ...
      infraSections[1],
      {
        ...
        list: ['You call — a live operator assesses the situation', 'Plumber sent with camera and hydro-jet equipment', 'Camera inspection performed before any clearing begins', 'Blockage cause identified — root, grease, scale, or damage', 'Appropriate clearing method selected and approved by you', 'Post-clearing camera confirms the line is fully open'],
      },
    ],
    blocks: getServiceBlocks('drain-cleaning', cityName, stateCode, d),
...
const BURST_FAQ_POOL = [
  { q: (c,s) => `Where is the main water shut-off valve in my ${c} home?`,
    a: (c,s) => `In most ${c} homes, the main shut-off is located where the water supply line enters the structure — typically in the basement, utility room, crawl space, or near the exterior water meter. If you're unsure, our operator can help you locate it over the phone while a plumber is en route.` },
  { q: (c,s) => `How fast can you respond to a burst pipe in ${c}?`,
    a: (c,s) => `Burst pipe calls in ${c} receive emergency priority. Our target is to have a plumber at your door within 60 minutes. We operate 24 hours a day, 7 days a week, and there are no additional charges for after-hours response.` },
  ...
  return {
    ...
    sections: [
      ...
      infraSections[1],
      {
        heading: h3,
        body: `${trustPhrase} ${pricingPhrase} ${responsePhrase}`,
        list: ['Licensed and insured plumbers in ' + (stateCode || 'your state'), 'Emergency service available at any hour', 'Stocked vehicles carry most common pipe materials and fittings', 'Upfront written quote before work begins', 'Post-repair pressure test confirms complete fix', 'Insurance documentation provided on request'],
      },
    ],
    blocks: getServiceBlocks('pipe-burst-repair', cityName, stateCode, d),
...
function genWaterHeater(cityName, stateCode, service) {
  ...
  return {
    ...
    blocks: getServiceBlocks('water-heater-repair', cityName, stateCode, d),
...
const SEWER_FAQ_POOL = [
  ...
  { q: (c,s) => `Do you handle sewer line emergencies in ${c}?`,
    a: (c,s) => `Yes. Sewer backups are considered plumbing emergencies and we prioritize them accordingly. If you have sewage backing up into your home, call immediately and we'll send a plumber as quickly as possible to stop the backup and begin repair.` },
...
function genSewerLineRepair(cityName, stateCode, service) {
  ...
  return {
    ...
    blocks: getServiceBlocks('sewer-line-repair', cityName, stateCode, d),
...
const TOILET_FAQ_POOL = [
  ...
  { q: (c,s) => `Do you handle toilet emergencies in ${c}?`,
    a: (c,s) => `Yes. Toilets that are overflowing, leaking significantly, or completely non-functional are considered urgent. We prioritize these calls and can typically send a plumber the same day to prevent water damage and restore functionality.` },
...
function genToiletRepair(cityName, stateCode, service) {
  ...
  return {
    ...
    blocks: getServiceBlocks('toilet-repair', cityName, stateCode, d),
...
const SLAB_FAQ_POOL = [
  ...
  { q: (c,s) => `Do you handle slab leak emergencies in ${c}?`,
    a: (c,s) => `Yes. Active slab leaks causing water damage are considered emergencies. We prioritize these calls and can typically respond within the same response window as other emergencies to minimize damage and begin repair quickly.` },
...
function genSlabLeakRepair(cityName, stateCode, service) {
  ...
  return {
    ...
    blocks: getServiceBlocks('slab-leak-repair', cityName, stateCode, d),
...
const WATERLINE_FAQ_POOL = [
  ...
  { q: (c,s) => `Do you handle water line emergencies in ${c}?`,
    a: (c,s) => `Yes. Water service line leaks that are causing significant water loss or low pressure are considered urgent. We prioritize these calls and can typically respond quickly to isolate the leak and begin repair, especially if water damage is occurring.` },
...
function genWaterLineRepair(cityName, stateCode, service) {
  ...
  return {
    ...
    blocks: getServiceBlocks('water-line-repair', cityName, stateCode, d),
...
const FAUCET_FAQ_POOL = [
  ...
  { q: (c,s) => `Do you handle faucet emergencies in ${c}?`,
    a: (c,s) => `Yes. Faucets that are spraying water, leaking significantly, or causing water damage are considered urgent. We prioritize these calls and can typically send a plumber same-day to stop the water damage and repair or replace the faucet.` },
  ...
  { q: (c,s) => `What should I do if my faucet breaks off in ${c}?`,
    a: (c,s) => `First, shut off the water supply using the shutoff valves under the sink. Don't force the handle or try to repair it yourself. Call a plumber immediately — a broken faucet can cause significant water damage quickly. We can typically send a plumber same-day for this type of emergency.` },
...
function genFaucetRepair(cityName, stateCode, service) {
  ...
  return {
    ...
    blocks: getServiceBlocks('faucet-repair', cityName, stateCode, d),
...
function genGarbageDisposalRepair(cityName, stateCode, service) {
  ...
  return {
    ...
    blocks: getServiceBlocks('garbage-disposal-repair', cityName, stateCode, d),
...
function genWaterSoftenerRepair(cityName, stateCode, service) {
  ...
  return {
    ...
    blocks: getServiceBlocks('water-softener-repair', cityName, stateCode, d),
...
const REPIPING_FAQ_POOL = [
  { q: (c) => `How do I know whether my ${c} home needs whole-house repiping?`, a: (c) => `Repeated pinhole leaks, rust-colored water, declining pressure at multiple fixtures, and repairs occurring on several pipe runs can indicate that replacing isolated sections is no longer the durable option. A repiping assessment maps the condition of the accessible system before a recommendation is made.` },
  { q: (c) => `Is repiping better than repairing another leak in ${c}?`, a: (c) => `It depends on the age, material, and pattern of failures in your plumbing. A single accessible leak often merits repair; repeated failures across aging lines can make repiping the more predictable long-term scope. We explain both options in writing.` },
  { q: (c) => `What pipe materials are used for a whole-house repipe in ${c}?`, a: (c) => `PEX and copper are common residential repiping materials. The right option depends on the existing system, local code requirements, routing access, water conditions, and your project goals. A technician can explain the trade-offs for your home.` },
  { q: (c) => `How long does whole-house repiping take in ${c}?`, a: (c) => `A typical home repipe often takes several days, though timing depends on home size, fixture count, access, pipe routing, inspection requirements, and restoration scope. Before work begins, you receive a project plan that identifies expected water interruptions.` },
  { q: (c) => `Will a repipe require opening walls in my ${c} home?`, a: (c) => `Some access openings are often necessary, but routing can sometimes reduce the amount of finished-surface disruption. The required openings depend on the home's layout and existing pipe paths. The assessment should document access and restoration responsibilities before work starts.` },
  { q: (c) => `Can repiping improve water pressure in ${c} homes?`, a: (c) => `It can when restricted or corroded interior piping is the source of low flow. A pressure and fixture-flow evaluation should first rule out a closed valve, failing regulator, aerator buildup, or utility-side issue before repiping is recommended.` },
  { q: (c) => `Does homeowners insurance cover repiping in ${c}?`, a: (c) => `Coverage depends on the policy and cause of loss. Sudden damage may be treated differently from gradual wear or pre-existing corrosion. We provide a documented scope and invoice that you can review with your insurer.` },
  { q: (c) => `Can you repipe only part of my ${c} home?`, a: (c) => `Yes. A targeted reroute or branch replacement may be appropriate when the rest of the plumbing is sound. The recommendation should be based on failure history, material condition, pressure testing, and access rather than a one-size-fits-all approach.` },
  { q: (c) => `What should I do before a repiping project in ${c}?`, a: (c) => `Clear access around mechanical areas and fixtures, protect valuables near planned work zones, and review the water-shutoff schedule and restoration scope with the project lead. Ask for the written material, routing, and access plan before work begins.` },
  { q: (c) => `How much does whole-house repiping cost in ${c}?`, a: (c) => `Costs vary materially with home size, fixture count, material, access, routing, permits, and restoration. A written assessment is the reliable way to compare a targeted repair, reroute, and complete repipe for your home.` },
];

const SHUTOFF_VALVE_FAQ_POOL = [
  { q: (c) => `Where is the main water shutoff valve in a typical ${c} home?`, a: (c) => `It is often located where the water service enters the home, near the water meter, in a utility area, crawl space, basement, garage, or exterior wall. Location varies by building layout, so identify and test it before an emergency occurs.` },
  { q: (c) => `What should I do if the main shutoff valve will not close in ${c}?`, a: (c) => `Avoid forcing a seized valve because a weakened stem or fitting can fail. If water is actively leaking, contact emergency plumbing service and, when appropriate, the water utility for service-side isolation. A technician can assess the safe replacement approach.` },
  { q: (c) => `How do I know whether my ${c} main shutoff valve needs replacement?`, a: (c) => `Leaks around the stem or body, a handle that will not turn, incomplete shutoff, visible corrosion, or repeated dripping after operation are signs the valve needs professional assessment. The priority is confirming that water can be isolated reliably.` },
  { q: (c) => `Can a plumber replace a corroded main shutoff valve in ${c}?`, a: (c) => `Yes. The work starts by identifying the valve's location, pipe material, ownership boundary, and safe isolation method. Corroded connections may require cutting and rebuilding adjacent pipe, which is why an on-site assessment matters.` },
  { q: (c) => `Is a ball valve better than an older gate valve in ${c} homes?`, a: (c) => `Ball valves generally provide a clear quarter-turn operating position and are commonly used for reliable isolation. The appropriate replacement still depends on pipe material, access, local code requirements, and the condition of the surrounding system.` },
  { q: (c) => `Can I replace a main water shutoff valve myself in ${c}?`, a: (c) => `Main shutoff work can affect the entire home's water supply and may involve utility coordination, corroded pipe, or code requirements. Professional installation helps ensure the valve is accessible, correctly sized, and tested before water service is restored.` },
  { q: (c) => `Does the water utility replace the shutoff valve for my ${c} property?`, a: (c) => `Responsibility often changes at the meter or service connection, but the exact boundary varies. A plumber can identify the affected valve and advise whether utility coordination is required before any work is scheduled.` },
  { q: (c) => `How long does main shutoff valve replacement take in ${c}?`, a: (c) => `An accessible replacement can often be completed in one visit. Timing increases when access is limited, connections are corroded, the water must be isolated at the service, or adjacent piping requires repair.` },
  { q: (c) => `How much does main water shutoff valve repair cost in ${c}?`, a: (c) => `Cost depends on the valve location, pipe material, corrosion, access, and isolation requirements. An accessible interior valve is usually simpler than a meter-adjacent or seized valve requiring additional pipe work.` },
  { q: (c) => `Why should I test my main shutoff valve in ${c}?`, a: (c) => `A working shutoff valve limits damage during a burst pipe, failed supply line, or appliance leak. Testing it periodically and replacing a valve that will not fully close provides a practical layer of emergency preparedness.` },
];

function genWholeHouseRepiping(cityName, stateCode, service) {
  const d = getCityData(cityName);
  const svc = 'whole-house-repiping';
  const pipeCondition = d.infraClass === 'aging'
    ? `The city's aging-infrastructure profile and ${d.pipeMaterial} from ${d.pipeEra} make a full-system assessment especially useful when repairs are recurring.`
    : d.infraClass === 'mixed'
    ? `Because ${cityName} has both established and newer housing, the decision should follow the condition of the individual home's ${d.pipeMaterial}, failure history, and routing access.`
    : `Even in newer housing, a repiping decision should be based on verified failure history, material condition, and accessibility rather than age alone.`;
  const climatePlan = d.winterRisk === 'high'
    ? `With winter lows averaging ${d.avgWinterTempF}°F, replacement planning also prioritizes protecting vulnerable runs in unconditioned spaces.`
    : `Local seasonal conditions, including ${d.summerRiskNote}, are considered when routing replacement lines and protecting accessible connections.`;
  const dispatchIntro = pick(DISPATCH_INTROS, cityName, svc, 'di')(cityName);
  const trustPhrase = pick(TRUST_PHRASES, cityName, svc, 'tp')(stateCode || 'your state');
  const h1 = pick(H_SERVICE_OVERVIEW, cityName, svc, 'h1')('Whole-House Repiping', cityName);
  const h2 = pick(H_WHY_US, cityName, svc, 'h2')(cityName);
  const h3 = pick(H_HOW_IT_WORKS, cityName, svc, 'h3')(cityName);
  const infraSections = buildInfraSection(cityName, stateCode, service);
  return {
    localSignal: buildLocalSignal(cityName, d),
    intro: `${dispatchIntro} Whole-house repiping replaces deteriorating interior supply piping when repeated spot repairs no longer offer a dependable solution. ${pipeCondition} ${climatePlan}`,
    sections: [
      { heading: h1, body: `Our ${cityName} repiping assessments distinguish between an isolated repair, targeted reroute, and full replacement. We review the existing ${d.pipeMaterial}, pipe era (${d.pipeEra}), access paths, fixture count, and the documented pattern behind ${d.dominantFailure} before recommending a scope. ${trustPhrase}`, list: ['Whole-home pipe condition assessment', 'PEX and copper material comparisons', 'Targeted reroutes and branch replacement', 'Pressure testing and fixture-flow checks', 'Written access and restoration planning', 'Final system testing after installation'] },
      infraSections[0], infraSections[1],
      { heading: h2, body: `A repipe is a planned infrastructure project, not a generic repair. We use the home's verified pipe condition and access constraints to explain the trade-off between continued repairs and replacement, with a written scope before work begins.`, list: [`Assessment of ${d.pipeMaterial} from the ${d.pipeEra}`, `Routing approach matched to the home's accessible areas`, 'Clear water-interruption schedule', 'Upfront written pricing and scope', 'Final pressure and leak testing', 'Warranty information for installed work'] },
      { heading: h3, body: `Whole-house repiping in ${cityName} begins with diagnosis, not assumptions. After documenting the system condition, we map replacement routes, explain material options, confirm access requirements, and coordinate the work sequence so the home is returned to service safely.`, list: ['Inspect accessible piping and prior repair areas', 'Review pressure, flow, and failure history', 'Select material and routing plan', 'Isolate water and install replacement lines', 'Test each connection and fixture', 'Review restoration and maintenance steps'] },
    ],
    blocks: getServiceBlocks('whole-house-repiping', cityName, stateCode, d),
    faqs: pickFaqs(REPIPING_FAQ_POOL, cityName, stateCode, service),
    pricing: buildPricingGuidance(cityName, svc),
  };
}

function genMainWaterShutoffValveRepair(cityName, stateCode, service) {
  const d = getCityData(cityName);
  const svc = 'main-water-shutoff-valve-repair';
  const pipeRisk = d.infraClass === 'aging'
    ? `Older ${d.pipeMaterial} installations from ${d.pipeEra} can leave valves and nearby fittings harder to operate or replace safely.`
    : `The home's pipe material, access, and the condition of the existing valve determine the safe replacement approach.`;
  const climateRisk = d.winterRisk === 'high'
    ? `Reliable isolation is particularly important before freeze events in a city with winter lows averaging ${d.avgWinterTempF}°F.`
    : `Reliable water isolation helps limit damage from the local plumbing risks associated with ${d.summerRiskNote}.`;
  const dispatchIntro = pick(DISPATCH_INTROS, cityName, svc, 'di')(cityName);
  const trustPhrase = pick(TRUST_PHRASES, cityName, svc, 'tp')(stateCode || 'your state');
  const h1 = pick(H_SERVICE_OVERVIEW, cityName, svc, 'h1')('Main Water Shutoff Valve Repair', cityName);
  const h2 = pick(H_WHY_US, cityName, svc, 'h2')(cityName);
  const h3 = pick(H_HOW_IT_WORKS, cityName, svc, 'h3')(cityName);
  const infraSections = buildInfraSection(cityName, stateCode, service);
  return {
    localSignal: buildLocalSignal(cityName, d),
    intro: `${dispatchIntro} A working main water shutoff valve is the first control point when a pipe, appliance line, or fitting fails. ${pipeRisk} ${climateRisk}`,
    sections: [
      { heading: h1, body: `Our ${cityName} technicians diagnose main shutoff valves that leak, seize, will not fully close, or are difficult to access. We identify the water-isolation point, inspect the adjacent ${d.pipeMaterial}, and determine whether repair or replacement is appropriate before any work begins. ${trustPhrase}`, list: ['Main shutoff valve testing and diagnosis', 'Leaking and seized valve replacement', 'Ball-valve upgrades where appropriate', 'Emergency water isolation', 'Meter-adjacent coordination when required', 'Post-repair operation and leak testing'] },
      infraSections[0], infraSections[1],
      { heading: h2, body: `A failed shutoff valve turns a manageable leak into a larger emergency. We document the valve location, isolate water safely, and verify the replacement closes and opens correctly before restoring service.`, list: ['Accessible valve assessment', 'Safe isolation before cutting or replacement', 'Pipe-material-compatible fittings', 'Written scope and upfront estimate', 'Operation test at full closure and reopening', 'Guidance on locating the valve in an emergency'] },
      { heading: h3, body: `Main water shutoff valve work in ${cityName} starts by confirming which valve controls the home's water and whether any service-side coordination is necessary. The technician then evaluates corrosion, access, and the surrounding pipe before completing and testing the approved repair.`, list: ['Locate and test the existing isolation point', 'Inspect valve body, stem, and nearby pipe', 'Confirm the required isolation method', 'Replace or repair the approved component', 'Restore water and check for leaks', 'Show the homeowner safe valve operation'] },
    ],
    blocks: getServiceBlocks('main-water-shutoff-valve-repair', cityName, stateCode, d),
    faqs: pickFaqs(SHUTOFF_VALVE_FAQ_POOL, cityName, stateCode, service),
    pricing: buildPricingGuidance(cityName, svc),
  };
}
...
const SUMP_PUMP_FAQ_POOL = [
  ...
  { q: (c,s) => `How fast can you respond to a failed sump pump in ${c}?`,
    a: (c,s) => `Sump pump failures are treated as emergencies in ${c} because of the risk of basement flooding. We respond immediately and aim to have a plumber at your home within 60 minutes to prevent water damage.` },
  ...
  { q: (c,s) => `What should I do if my basement floods in ${c}?`,
    a: (c,s) => `First, ensure electrical safety — do not enter standing water if electrical outlets or appliances are submerged. Call us immediately for emergency water removal and sump pump repair. We can respond quickly to minimize damage and restore your pump system.` },
...
function genSumpPumpRepair(cityName, stateCode, service) {
  ...
  return {
    ...
    blocks: getServiceBlocks('sump-pump-repair', cityName, stateCode, d),
