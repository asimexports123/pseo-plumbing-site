...
import { getPlaceBySlug, getGenericCityData, getAllPlaces } from './nationwidePlaces';
...
export function getCityData(cityName) {
  ...
  const place = allPlaces.find(p => p.name === cityName);
  if (place) {
    return getGenericCityData(cityName, place.stateCode);
  }
  // Last resort: check if it's a SEED_CITIES entry (has stateCode)
  const seedCity = SEED_CITIES.find(c => c.name === cityName);
  if (seedCity) {
    return getGenericCityData(cityName, seedCity.stateCode);
  }
  throw new Error(
