...
export const DISPATCH_INTROS = [
  ...
  (c) => `${c} homes face specific plumbing vulnerabilities — fast response is the first line of defense.`,
];

export const HOW_WE_WORK_INTROS = [
  (c) => `Getting help in ${c} is straightforward: call, describe the issue, and a plumber is on the way immediately.`,
  (c) => `Our ${c} service process is built for speed — from your call to a plumber en route in under five minutes.`,
  (c) => `${c} residents receive same-day service on most calls, with live operator support from first contact to job completion.`,
  (c) => `Calling our ${c} line puts you directly in touch with a live operator who ensures a plumber is on the way quickly.`,
  (c) => `We've structured our ${c} service process to eliminate every delay between your call and the moment repairs begin.`,
  (c) => `From the moment you call about a ${c} plumbing emergency, our team takes immediate action.`,
  (c) => `Our ${c} service model prioritizes response time above all — a live operator answers every call, day or night.`,
  (c) => `${c} customers don't navigate phone trees or wait on hold — every call goes directly to a live operator.`,
];

export const TRUST_PHRASES = [
  (st) => `All plumbers serving ${st} are fully licensed, insured, and background-checked.`,
  (st) => `Every plumber serving ${st} customers carries a valid state license and active proof of insurance.`,
  (st) => `We only use ${st}-licensed plumbers who meet our stringent vetting and insurance requirements.`,
  (st) => `Plumbers working in ${st} are required to hold active state licenses and carry liability coverage.`,
  (st) => `Every plumber we work with in ${st} is licensed, insured, and independently verified.`,
  (st) => `Before any ${st} plumber begins work, we confirm their license status, insurance, and service record.`,
  (st) => `Every ${st} service call involves a vetted professional — licensed, bonded, and insured for your protection.`,
...
export const RESPONSE_PHRASES = [
  ...
  (c) => `For emergencies in ${c}, a plumber is typically on the way within five minutes of your call.`,
  (c) => `${c} emergency calls receive priority — our target arrival window is under one hour.`,
  (c) => `When you call from ${c}, we send the nearest available licensed plumber to your address immediately.`,
  (c) => `Our ${c} service area is structured to minimize response delays.`,
  (c) => `Speed matters in a ${c} plumbing emergency — we work to have someone at your door as fast as possible.`,
  (c) => `${c} calls are handled by a team that prioritizes minimal wait times on every job.`,
];
...
export const H_HOW_IT_WORKS = [
  (c) => `How Our ${c} Service Process Works`,
