...
let placesByState = {};
let _totalPlaces = 0;
let _coveredStates = null;
let _nationwideServiceCounts = null;
let _loadingPromise = null;

// ── Sharded loading state (per-state) ─────────────────────────
let _slugStateMap = null;
let _placesMeta = null;
let _loadedStates = new Set();
let _metaLoadingPromise = null;
let _stateShardPromises = {};

export async function ensurePlacesLoaded() {
  if (usPlacesData) return;
  if (_loadingPromise) return _loadingPromise;
  _loadingPromise = (async () => {
    let data;
    try {
      data = _readJsonSync(
        (process.cwd() + '/data/us_places.json')
      );
    } catch {
      const { getCloudflareContext } = await import('@opennextjs/cloudflare');
      const ctx = await getCloudflareContext({ async: true });
      const res = await ctx.env.ASSETS.fetch(new Request('https://local/us-places-full.json'));
      data = await res.json();
    }
    usPlacesData = data;
    // Build a lookup map: slug -> place object
    slugMap = {};
    data.forEach(p => { slugMap[p.slug] = p; });
    // Build a lookup map: city name -> place object
    // Note: duplicate city names across states resolve to the last entry,
    // which matches the previous find() behaviour (returns first match in
    // array order — but callers that need state-specific lookup should use
    // getPlaceByNameState). For getCityData() in cities-server.js, the
    // fallback path first checks CITY_DATA, then SEED_CITIES, then this map.
    nameMap = {};
    data.forEach(p => { if (!(p.name in nameMap)) nameMap[p.name] = p; });
    // Build a lookup map: "State Code:City Name" -> place object
    nameStateMap = {};
    data.forEach(p => { nameStateMap[p.stateCode + ':' + p.name] = p; });
    // Pre-compute places grouped by state code for O(1) state lookups
    placesByState = {};
    data.forEach(p => {
      if (!placesByState[p.stateCode]) placesByState[p.stateCode] = [];
      placesByState[p.stateCode].push(p);
    });
    _totalPlaces = data.length;
    _coveredStates = [...new Set(data.map(p => p.stateCode))].sort();
    // Compute nationwide service counts
    const valid = data.filter(p => SUPPORTED_STATE_CODES.has(p.stateCode));
    const SVC_SLUGS = [
    ...
    SVC_SLUGS.forEach(slug => {
      if (slug === 'sump-pump-repair') {
        counts[slug] = valid.filter(p => SUMP_PUMP_QUALIFIED_STATE_CODES.has(p.stateCode)).length;
      } else {
        counts[slug] = valid.length;
      }
    });
    _nationwideServiceCounts = counts;
  })();
  return _loadingPromise;
}

// ── Sharded loading (per-state) ───────────────────────────────
// Loads only a small slug→stateCode index + per-state place shards.
// Eliminates parsing the full 2.66MB dataset at runtime.

export async function ensurePlacesMetaLoaded() {
  if (_placesMeta) return;
  if (_metaLoadingPromise) return _metaLoadingPromise;
  _metaLoadingPromise = (async () => {
    let meta, slugState;
    try {
      meta = _readJsonSync(
        process.cwd() + '/public/data/places_meta.json'
      );
      slugState = _readJsonSync(
        process.cwd() + '/public/data/places_slug_state.json'
      );
    } catch (fsErr) {
      const { getCloudflareContext } = await import('@opennextjs/cloudflare');
      const ctx = await getCloudflareContext({ async: true });
      const [metaRes, slugRes] = await Promise.all([
        ctx.env.ASSETS.fetch(new Request('https://local/data/places_meta.json')),
        ctx.env.ASSETS.fetch(new Request('https://local/data/places_slug_state.json')),
      ]);
      meta = await metaRes.json();
      slugState = await slugRes.json();
    }
    _placesMeta = meta;
    _slugStateMap = slugState;
  })();
  return _metaLoadingPromise;
}

export function getStateCodeForSlugSync(slug) {
  return _slugStateMap ? (_slugStateMap[slug] || null) : null;
}

export async function ensurePlacesForStateLoaded(stateCode) {
  if (!stateCode) return;
  if (_loadedStates.has(stateCode)) return;
  if (_stateShardPromises[stateCode]) return _stateShardPromises[stateCode];
  _stateShardPromises[stateCode] = (async () => {
    let data;
    try {
      const filePath = process.cwd() + '/public/data/places_state_' + stateCode + '.json';
      data = _readJsonSync(filePath);
    } catch (fsErr) {
      const { getCloudflareContext } = await import('@opennextjs/cloudflare');
      const ctx = await getCloudflareContext({ async: true });
      const res = await ctx.env.ASSETS.fetch(new Request(`https://local/data/places_state_${stateCode}.json`));
      data = await res.json();
    }
    if (!usPlacesData) usPlacesData = [];
    data.forEach(p => {
      usPlacesData.push(p);
      slugMap[p.slug] = p;
      if (!(p.name in nameMap)) nameMap[p.name] = p;
      nameStateMap[p.stateCode + ':' + p.name] = p;
    });
    placesByState[stateCode] = data;
    _loadedStates.add(stateCode);
  })();
  return _stateShardPromises[stateCode];
}

export function getTotalPlacesFromMetaSync() {
  return _placesMeta ? _placesMeta.totalPlaces : 0;
}

export function getNationwideServiceCountsFromMetaSync() {
  return _placesMeta ? _placesMeta.nationwideServiceCounts : null;
}
...
// Get place by name (first match in dataset order)
...
export async function getPlaceByNameState(name, stateCode) {
  await ensurePlacesLoaded();
  return nameStateMap[stateCode + ':' + name] || null;
}

// Get all places for a state (O(1) via pre-computed map)
export async function getPlacesByState(stateCode) {
  await ensurePlacesLoaded();
  return placesByState[stateCode] || [];
}
...
export async function getCoveredStates() {
  await ensurePlacesLoaded();
  return _coveredStates;
}

// Nationwide service counts
export async function getNationwideServiceCounts() {
  await ensurePlacesLoaded();
  return _nationwideServiceCounts;
}
...
export async function getNearbyPlaces(slug, stateCode, limit = 8) {
  await ensurePlacesLoaded();
  ...
  const sameState = (placesByState[stateCode] || []).filter(p =>
    p.slug !== slug &&
    p.lat && p.lon
  );
...
// Pre-build the N largest places by land area for static generation.
// Larger land area correlates with population/importance, so these are the
// pages most likely to be hit as first requests.
export async function getTopPlacesByLandArea(limit = 1000) {
  await ensurePlacesLoaded();
  return [...usPlacesData]
    .sort((a, b) => (b.aland || 0) - (a.aland || 0))
    .slice(0, limit);
}

// ── Sync variants (use after ensurePlacesLoaded() has been awaited) ──

export function getPlaceBySlugSync(slug) {
  return slugMap[slug] || null;
}

export function getPlaceByNameSync(name) {
  return nameMap[name] || null;
}

export function getPlaceByNameStateSync(name, stateCode) {
  return nameStateMap[stateCode + ':' + name] || null;
}

export function getPlacesByStateSync(stateCode) {
  return placesByState[stateCode] || [];
}

export function getTotalPlacesSync() {
  return _totalPlaces;
}

export function getNearbyPlacesSync(slug, stateCode, limit = 8) {
  const source = slugMap[slug];
  if (!source || !source.lat || !source.lon) {
    return (placesByState[stateCode] || [])
      .filter(p => p.slug !== slug)
...
export function getNationwideServiceCountsSync() {
  return _nationwideServiceCounts;
}
